(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.VuePrlx = {}));
    ...
    function onUnbind(el) {
        if (!el.__prlxRequestAnimationFrameId)
            return;
    ...
    function animate(el, scrollPosition, settings) {
        ...
        // NORMALIZE OFFSET
        if (settings.limit) {
            if (offset > settings.limit.max)
                offset = settings.limit.max;
            if (offset < settings.limit.min)
                offset = settings.limit.min;
        }
        // RUN PARALLAX
        let parallaxType;
        if (settings.background) {
